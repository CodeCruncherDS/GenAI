...
import { getProfessionalProject, getProfessionalProjectPageContent } from "@/lib/content-loader";

export default async function ProfessionalProjectPage({ params }: { params: { slug: string } }) {
  const [project, content] = await Promise.all([
    getProfessionalProject(params.slug),
    getProfessionalProjectPageContent()
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-sand-200 bg-white/80 p-8 shadow-soft">
        <h2 className="text-xl font-display text-ink-900">{content.overviewTitle}</h2>
        <div className="mt-4">
